...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentence.split(' ').length; i += 1) {
    if (sentence.split(' ')[i].length > len) {
      len = sentence.split(' ')[i].length;
...
function invertCase(str) {
  return str
    .split('')
    .map((x) =>
      x === x.toLowerCase() ? x.toUpperCase() : x.toLocaleLowerCase()
    )
    .join('');
}
...
function encodeToRot13(str) {
  ...
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    if (source.includes(str[i])) {
      result += target[source.indexOf(str[i])];
    } else {
      result += str[i];
    }
  }
